...
//      // <Study seed thresholds>

//      l1slhc::L1CaloTowerRefVector mConstituents = Tower_It->getConstituents();

//      //     int maxTTEnergy        = 0;
//      double TTGCTSeedRegionEnergy = 0;

//      for ( l1slhc::L1CaloTowerRefVector::const_iterator lConstituentIt = mConstituents.begin() ; lConstituentIt != mConstituents.end(); ++lConstituentIt ){

//        double jetCentreiPhi = Tower_It->iPhi() + Tower_It->JetSize()/2;
//        double jetCentreiEta = Tower_It->iEta() + Tower_It->JetSize()/2;
// //        double jetGeoCentrePhi  = Tower_It->Phi();
// //        double jetGeoCentreEta  = Tower_It->Eta();
//        double TTiPhi = (**lConstituentIt).iPhi();
//        double TTiEta = (**lConstituentIt).iEta();

//        double deltaiPhi = jetCentreiPhi - TTiPhi;
//        double deltaiEta = jetCentreiEta - TTiEta;

//        double TTEnergy = 0.5*((**lConstituentIt).E() + (**lConstituentIt).H());

//        // Check if the TT is within the central GCT seed region
//        if ( ( fabs( deltaiPhi ) <= 2) && ( fabs( deltaiEta ) <= 2) ){
	 
// 	 TTGCTSeedRegionEnergy += TTEnergy;

//        }

// //        int TTEnergy = (**lConstituentIt).E() + (**lConstituentIt).H();
// //        if ( TTEnergy > maxTTEnergy ){
// // 	 maxTTEnergy = TTEnergy;
// //        }

// //        std::cout << TTGCTSeedRegionEnergy << "\t(" << jetCentreiEta << ", " << jetCentreiPhi << ")\t(" << TTiEta << ", " << TTiPhi << ")\t" 
// // 		  << TTEnergy << "\n";
...
// //      std::cout << "\nFinal region energy : " << TTGCTSeedRegionEnergy << "\n";
     
// //      std::cout << "----------------------------------------\n";
     
//      // Apply GCT seed threshold
//      if ( TTGCTSeedRegionEnergy < 5 ){
//        //       std::cout << "SKIPPED JET\n";
//        continue;
//      }
...
//      TLorentzVector *tempJet2;
//      tempJet2->SetPtEtaPhiM( Tower_It->p4().Pt(), Tower_It->p4().eta(), Tower_It->p4().phi(), Tower_It->p4().M() );
//      outputLPUSTowerJet_TLorentz->push_back( tempJet2 );
...
//    std::cout << "--------------------------------------------------------------------------------\n";
   ...
   //
   for (reco::CaloJetCollection::const_iterator PrePUSRawak5_It = PrePUSRawAk5Jets->begin(); PrePUSRawak5_It != PrePUSRawAk5Jets->end(); ++PrePUSRawak5_It ){
...
//    iEvent.put( outputPUS_Tower_TLorentz, "PUS_Tower" );
   ...
   // Jets
   iEvent.put( outputPrePUSTowerJet_L1Jet,     "PrePUSTowerJetL1Jet");
   iEvent.put( outputPUSTowerJet_L1Jet,        "PUSTowerJetL1Jet");
   iEvent.put( outputLPUSTowerJet_L1Jet,       "LPUSTowerJetL1Jet");
   iEvent.put( outputUncalibCurrentJet_L1Jet,  "CurrentUncalibJetL1Jet");
   iEvent.put( outputCurrentJet_L1Jet,         "CurrentJetL1Jet");
   iEvent.put( outputPrePUSRawAk5CaloJet_L1Jet,"PrePUSRawAk5CaloJetL1Jet");
   ...
   iEvent.put( outputAk5CaloJet_L1Jet,         "Ak5CaloJetL1Jet");
