...
//   produces<reco::CaloJetCollection>( "TTKt6CaloJet" );
  produces<reco::CaloJetCollection>( "CaloAk5CaloJet" );
  produces<reco::CaloJetCollection>( "CaloKt6CaloJet" );
...
//   std::auto_ptr<reco::CaloJetCollection> outputTTAk5_CaloJet(   new reco::CaloJetCollection() );
//   std::auto_ptr<reco::CaloJetCollection> outputTTKt6_CaloJet(   new reco::CaloJetCollection() );
  std::auto_ptr<reco::CaloJetCollection> outputCaloAk5_CaloJet( new reco::CaloJetCollection() );
  std::auto_ptr<reco::CaloJetCollection> outputCaloKt6_CaloJet( new reco::CaloJetCollection() );
...
//     double Phi = mTowerGeo.phi(iPhi);
  ...
  AreaDefinition Ak5_area_def(active_area_explicit_ghosts,GhostedAreaSpec(SelectorAbsRapMax(3.0)));

  ClusterSequenceArea csAk5Calo(particlesCalo, Ak5, Ak5_area_def );
  ...
  for (unsigned i = 0; i < Ak5CaloJets.size(); i++) {
    ...
    double area = csAk5Calo.area( Ak5CaloJets[i] );

    reco::CaloJet tempCaloJet;


    ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<double> > jetVec;
    jetVec.SetCoordinates( Pt, Eta, Phi, M );
    // Dummy data to allow use of constructor
    reco::CaloJet::Specific defaultSpecfic;
    math::XYZPoint          defaultPoint;


    tempCaloJet = reco::CaloJet( jetVec, defaultPoint, defaultSpecfic );
    tempCaloJet.setJetArea( area );

     outputCaloAk5_CaloJet->push_back( tempCaloJet );

     //     std::cout << area << "  " << Pt << "   " << Eta << "  " << Phi << std::endl;
  ...
  iEvent.put( outputCaloAk5_CaloJet, "CaloAk5CaloJet");
